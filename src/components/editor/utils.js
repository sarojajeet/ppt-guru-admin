...
// };
export function containsLatex(str) {
  return /\\\(|\\\[|\$/.test(str);
}
...
// };
export const parseSlides = (rawData) => {
    const type = detectInputType(rawData);

    switch (type) {
        case 'slides':
            return normalizeSlides(parseSlideArray(rawData));

        case 'sections':
            return normalizeSlides(parseSections(rawData));

        case 'text':
            return normalizeSlides(parseText(rawData));

        default:
            return [defaultSlide(0)];
    }
};
const detectInputType = (data) => {
    if (!data) return 'empty';

    // ✅ Already structured slides
    ...
    if (typeof data === 'object') {
        const sections = data.sections || data.aiContent?.sections;
...
const parseSlideArray = (slides) => {
    return slides.map((slide, idx) => ({
        ...slide,
        id: slide.id || `slide_${idx}`,
...
const parseSections = (data) => {
    const sections = data.sections || data.aiContent?.sections || [];
    const title = data.title || data.aiContent?.title || '';
    ...
    if (title) {
        ...
        s.elements = [
            ...
            createTextElement(
                '<p>Generated by NeuralNotes AI</p>',
                50,
                200,
                { w: 1020, h: 60, fontSize: 20 }
            ),
        ];
        slides.push(s);
    }
